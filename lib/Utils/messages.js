...
const buildRichResponse = (message) => {
    const proto = WAProto_1.proto;
    const Type = proto.AIRichResponseMessage.AIRichResponseSubMessageType;
    const submessages = [];
    const text = message.text || (message.richResponse && message.richResponse.text);
    if (text) {
        submessages.push({
            messageType: Type.AI_RICH_RESPONSE_TEXT,
            messageText: String(text)
        });
    }
    if (message.code) {
        const code = typeof message.code === 'string' ? { content: message.code } : message.code;
        submessages.push({
            messageType: Type.AI_RICH_RESPONSE_CODE,
            codeMetadata: {
                codeLanguage: code.language || code.codeLanguage || 'text',
                codeBlocks: [{ codeContent: code.content || code.code || '' }]
            }
        });
    }
    if (message.latex) {
        const latex = typeof message.latex === 'string' ? message.latex : (message.latex.text || '');
        submessages.push({
            messageType: Type.AI_RICH_RESPONSE_LATEX,
            latexMetadata: { text: latex }
        });
    }
    if (message.table) {
        submessages.push({
            messageType: Type.AI_RICH_RESPONSE_TABLE,
            tableMetadata: {
                title: message.table.title || '',
                rows: (message.table.rows || []).map((row) => ({
                    items: Array.isArray(row) ? row.map(String) : (row.items || []).map(String),
                    isHeading: !!(row && row.isHeading)
                }))
            }
        });
    }
    if (message.richResponse && Array.isArray(message.richResponse.submessages)) {
        submessages.push(...message.richResponse.submessages);
    }
    return {
        messageType: proto.AIRichResponseMessage.AIRichResponseMessageType.AI_RICH_RESPONSE_TYPE_STANDARD,
        submessages
    };
};
const generateWAMessageContent = async (message, options) => {
    ...
    var _b;
    let m = {};
    if (message && ('code' in message || 'table' in message || 'latex' in message || 'richResponse' in message || 'html' in message || 'map' in message || 'imageUrl' in message)) {
        m.richResponseMessage = buildRichResponse(message);
    }
