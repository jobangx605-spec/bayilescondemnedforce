...
const makeGroupsSocket = (config) => {
    ...
    const groupMetaTtl = config.groupMetadataCacheTtlMs || 300000;
    const groupMetadata = async (jid, force = false) => {
        if (!jid) {
            throw new Error('group jid required');
        }
        if (force) {
            ...
            const pending = groupMetaInflight.get(jid);
            if (pending) {
                return pending;
            }
        }
        const task = (async () => {
            ...
            groupMetaCache.set(jid, { value: meta, exp: Date.now() + groupMetaTtl });
            return meta;
        })();
        groupMetaInflight.set(jid, task);
        try {
            return await task;
        }
        finally {
            if (groupMetaInflight.get(jid) === task) {
    ...
    const groupFetchAllParticipating = async () => {
        ...
        if (groupsChild) {
            ...
            for (const groupNode of groups) {
                ...
                data[meta.id] = meta;
                groupMetaCache.set(meta.id, { value: meta, exp: Date.now() + groupMetaTtl });
            }
        }
...
const extractGroupMetadata = (result) => {
    ...
    const metadata = {
        ...
        participants: WABinary_1.getBinaryNodeChildren(group, 'participant').map(({ attrs }) => {
            const lid = attrs.lid || (attrs.jid && String(attrs.jid).endsWith('@lid') ? attrs.jid : undefined);
            const phone = (0, Utils_1.toPhoneJid)(attrs.phone_number || attrs.pn);
            if (lid && phone) (0, Utils_1.rememberLid)(lid, phone);
            return {
                id: attrs.jid,
                jid: phone || attrs.jid,
                lid: lid ? (0, WABinary_1.jidNormalizedUser)(lid) : undefined,
                phoneNumber: phone,
