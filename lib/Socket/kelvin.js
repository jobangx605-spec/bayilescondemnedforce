...
class kelvin {
    ...
    detectType(content) {
        ...
        if (Array.isArray(content.album)) return 'ALBUM_V2';
...
async handleStickerPack(content, jid, quoted) {
    ...
    async handleAlbumV2(content, jid, userJid, options = {}) {
        const album = Array.isArray(content.album) ? content.album.slice() : [];
        if (!album.length) throw new Error('album harus array dan tidak boleh kosong');
        if (content.caption && album[0] && !album[0].caption) album[0] = { ...album[0], caption: content.caption };
        const albumMsg = await this.utils.generateWAMessageFromContent(jid, {
            albumMessage: {
                expectedImageCount: album.filter((item) => item && Object.prototype.hasOwnProperty.call(item, 'image')).length,
                expectedVideoCount: album.filter((item) => item && Object.prototype.hasOwnProperty.call(item, 'video')).length
            }
        ...
        for (const media of album) {
            let msgContent;
            if (media && Object.prototype.hasOwnProperty.call(media, 'image')) {
                msgContent = { image: media.image, ...(media.caption ? { caption: media.caption } : {}) };
            }
            else if (media && Object.prototype.hasOwnProperty.call(media, 'video')) {
                msgContent = {
                    video: media.video,
