...
const rememberInbound = (id) => {
    if (!id) return;
    inboundSeen.set(id, Date.now());
    if (inboundSeen.size <= 4000) return;
    const cutoff = Date.now() - (10 * 60 * 1000);
    for (const [key, at] of inboundSeen) {
        if (at < cutoff || inboundSeen.size > 3000) inboundSeen.delete(key);
        if (inboundSeen.size <= 3000) break;
    }
};
const noteInboundJid = (msg) => {
    const jid = msg && msg.key && msg.key.remoteJid;
    if (jid && !(msg.key && msg.key.fromMe) && sock.antiOverhit) {
        sock.antiOverhit.noteInbound(jid);
    }
};
...
const identityAssertDebounce = {
map: new Map(),
get(jid) {
const at = this.map.get(jid);
...
const identityResult = await Utils_1.handleIdentityChange(node, {
meId: authState.creds.me && authState.creds.me.id,
meLid: authState.creds.me && authState.creds.me.lid,
validateSession: (jid) => signalRepository.validateSession(jid),
assertSessions,
debounceCache: identityAssertDebounce,
logger
});
if (identityResult.action === 'no_identity_node') {
...
const found = await getMessage({ ...key, id });
if (found) return found;
const manager = sock.messageRetryManager;
if (!manager) return undefined;
const cached = manager.getRecentMessage(key.remoteJid, id) || manager.getRecentMessageById(id);
...
let msg, category, author, decrypt;
let acked = false;
let surfaced = false;
try {
({ fullMessage: msg, category, author, decrypt } = (0, Utils_1.decryptMessageNode)(node, authState.creds.me.id, authState.creds.me.lid || '', signalRepository, logger));
}
catch (decodeErr) {
logger.error({ decodeErr, node }, 'failed to decode incoming message');
acked = true;
...
await decrypt();
(0, Utils_1.applyMessageJid)(msg, node.attrs, authState.creds.me);
...
let type = undefined;
(0, Utils_1.applyMessageJid)(msg, node.attrs, authState.creds.me);
const senderStillLid = msg.key && msg.key.participant && String(msg.key.participant).endsWith('@lid');
if ((0, WABinary_1.isJidGroup)(msg.key.remoteJid) && senderStillLid) {
try {
const metadata = await Promise.race([
groupMetadata(msg.key.remoteJid),
new Promise((_, reject) => setTimeout(() => reject(new Error('group metadata timeout')), 1500))
]);
(0, Utils_1.rememberParticipants)(metadata && metadata.participants);
}
catch (err) {
logger.debug({ err, jid: msg.key.remoteJid }, 'group phone map skipped, message kept');
}
}
(0, Utils_1.applyMessageJid)(msg, node.attrs, authState.creds.me);
...
const inboundId = msg.key && `${msg.key.remoteJid}:${msg.key.id}:${msg.key.fromMe ? 1 : 0}`;
if (inboundId && inboundSeen.has(inboundId)) {
acked = true;
await sendMessageAck(node);
return;
}
...
logger.warn({ ackErr }, 'failed to ack message after handler error');
}
}
if (!surfaced && msg) {
try {
...
logger.warn({ upsertErr }, 'failed to surface message after handler error');
}
}
}
};
...
ev.emit('call', [call]);
if (antiCallEnabled && status === 'offer' && callId && from) {
const targets = [from];
if (call.isGroup && call.groupJid && call.groupJid !== from) targets.push(call.groupJid);
for (const target of targets) {
rejectCall(callId, target).catch((err) => logger.warn({ err, callId, target }, 'antiCall reject failed'));
}
}
await sendMessageAck(node);
};
...
ev.buffer();
try {
await Promise.resolve(exec(node, false)).catch(err => onUnexpectedError(err, identifier));
}
finally {
ev.flush();
}
};
const offlineNodeProcessor = Utils_1.createOfflineQueue({
isOpen: () => ws.isOpen,
onError: (error, identifier) => onUnexpectedError(error, identifier),
handlers: new Map([
...
if (connection === 'open' && antiCallEnabled && typeof sock.updateCallPrivacy === 'function') {
sock.updateCallPrivacy('none').catch((err) => logger.debug({ err }, 'antiCall privacy not applied'));
}
});
...
decodeJid: (jid) => (0, Utils_1.decodeJid)(jid),
setAntiCall: (enabled) => {
antiCallEnabled = enabled !== false;
