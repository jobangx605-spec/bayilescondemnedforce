...
await decrypt();
(0, Utils_1.applyMessageJid)(msg, node.attrs, authState.creds.me && authState.creds.me.id);
...
const found = metadata && metadata.participants && metadata.participants.find(p => p.id === sender);
msg.message.extendedTextMessage.contextInfo.participant = (found === null || found === void 0 ? void 0 : found.jid) || sender;
}
catch (err) {
logger.warn({ err, jid: msg.key.remoteJid }, 'group metadata lookup failed, message kept');
}
}
}
(0, Utils_1.applyMessageJid)(msg, node.attrs, authState.creds.me && authState.creds.me.id);
