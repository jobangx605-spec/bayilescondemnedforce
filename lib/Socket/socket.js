...
const makeSocket = (config) => {
    ...
    const lidMapping = new lid_mapping_1.LIDMappingStore(keys, logger);
    signalRepository.lidMapping = lidMapping;
    if (typeof signalRepository.validateSession !== "function") {
        signalRepository.validateSession = async (jid) => {
            const addr = signalRepository.jidToSignalProtocolAddress(jid);
            const sessions = await keys.get("session", [addr]);
            return { exists: !!(sessions && sessions[addr]) };
        };
    }
    const rateLimiter = new Utils_1.RateLimiter(config.rateLimiter || {});
    let lastDateRecv;
    let epoch = 1;
    let keepAliveReq;
    let qrTimer;
    ...
    const queryLane = Utils_1.createQueryLane({
    ...
    const query = async (node, timeoutMs) => {
        return queryLane.run(node, async () => {
            if (!node.attrs.id) {
    ...
    const end = (error) => {
        ...
        const reconnectHint = Utils_1.suggestReconnect(error);
        const statusCode = error && error.output && error.output.statusCode;
        const disconnectClass = Utils_1.classifyDisconnect(statusCode);
        ev.emit('connection.update', {
            ...
            reconnectDelayMs: reconnectHint.delayMs,
            shouldReconnect: reconnectHint.shouldReconnect,
            classifyDisconnect: disconnectClass
        });
    ...
    ws.on('CB:success', async (node) => {
        try {
            ...
            if (node.attrs.lid && authState.creds.me && authState.creds.me.id) {
                lidMapping.storeLIDPNMappings([{ lid: node.attrs.lid, pn: authState.creds.me.id }]).catch(() => {});
            }
