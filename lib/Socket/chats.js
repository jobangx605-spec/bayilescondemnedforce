...
const makeChatsSocket = (config) => {
    ...
    const updateCallPrivacy = async (value) => {
        const mapped = value === 'everyone' ? 'all' : (value === 'nobody' ? 'none' : value);
        await privacyQuery('calladd', mapped);
    };
    const updateMessagesPrivacy = async (value) => {
        const mapped = value === 'nobody' ? 'none' : value;
        await privacyQuery('messages', mapped);
    };
    const updateDisableLinkPreviewsPrivacy = async (disabled) => {
