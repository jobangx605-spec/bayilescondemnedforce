...
const binaryChildIndex = new WeakMap();
const childrenByTag = (node) => {
    if (!node || typeof node !== 'object') {
        return null;
    }
    const len = Array.isArray(node.content) ? node.content.length : 0;
    const cached = binaryChildIndex.get(node);
    ...
    binaryChildIndex.set(node, { len, map });
    return map;
};
const getBinaryNodeChildren = (node, childTag) => {
    const map = childrenByTag(node);
    if (!map) {
        return [];
    }
    return (map.get(childTag) || []).slice();
}
